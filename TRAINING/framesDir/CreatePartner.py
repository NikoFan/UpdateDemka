...
class CreatePartnerClass(QFrame):
    ...
    def setup_ui(self):
        ...
        self.partner_name = self.create_qline_edit_pattern("Наименование партнера", 100)
        ...
        self.partner_rate = self.create_qline_edit_pattern("Рейтинг от 1 до 10", 2)

        self.create_label_pattern("Введите юридический адрес партнера")
        self.partner_addr = self.create_qline_edit_pattern("Адрес партнера", 300)

        self.create_label_pattern("Введите ФИО Директора партнера")
        self.partner_dir = self.create_qline_edit_pattern("Иванов Иван Иванович", 100)

        self.create_label_pattern("Введите Телефон партнера")
        self.partner_phone = self.create_qline_edit_pattern("+7 ...", 13)
        self.partner_phone.setInputMask("+7 000 000 00 00")


        self.create_label_pattern("Введите Почту электронную партнера")
        self.partner_email = self.create_qline_edit_pattern("Введите Почту", 100)

        self.create_label_pattern("Введите ИНН партнера")
        self.partner_inn = self.create_qline_edit_pattern("Введите ИНН", 10)

        add_btn = QPushButton("Добавить")
        add_btn.clicked.connect(
            self.create_partner_info
        )
    ...
    def create_partner_info(self):
        ...
        if send_W_message("Проверьте введенные данные!") < 20_000:
            # Редактирование
            if self.database.create_partner_data(partner_input_data):
                send_I_message("Добавление прошло успешно!")
                return
    ...
    def create_qline_edit_pattern(self, hint_text, max_len):
        """
        Функция создания ввода для пользователя
        :param hint_text: Namek текст
